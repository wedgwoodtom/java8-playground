...
//
    ...
    {
        Predicate<Employee> startsWithA = e -> e.id < 10 && e.name.startsWith("A");
        Predicate<Employee> salaryLessthan10k = e -> e.sal < 10000;
        List<Employee> list = Employee.getEmpList();
        //using allMatch
        boolean b1 = list.stream().allMatch(startsWithA);
        System.out.println(b1);
        boolean b2 = list.stream().allMatch(salaryLessthan10k);
        System.out.println(b2);
        //using anyMatch
        boolean b3 = list.stream().anyMatch(startsWithA);
        System.out.println(b3);
        boolean b4 = list.stream().anyMatch(salaryLessthan10k);
        System.out.println(b4);
        //using noneMatch
        boolean b5 = list.stream().noneMatch(startsWithA);
    ...
    @Test
    public void testMapWithGroupingBy()
    {
        Map<String, List<Movie>> moviesByGenre =
                movieList.stream().collect(Collectors.groupingBy((Movie::getGenre)));

        Assert.assertTrue(moviesByGenre.size() >= 3);
        Assert.assertTrue(moviesByGenre.get("SciFi").size() >= 3);
